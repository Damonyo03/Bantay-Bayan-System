...
const DispatchBottomSheet: React.FC<Props> = ({ dispatchLog, availableOfficers, onClose, onUpdate }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      {/* Sheet */}
      <div className="relative w-full max-w-lg bg-white/90 backdrop-blur-xl rounded-t-3xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 transform transition-transform duration-300 ease-out animate-slide-up border border-white/20">
        
        {/* Handle for mobile feel */}
        <div className="w-12 h-1.5 bg-gray-300 rounded-full mx-auto mb-6 sm:hidden" />

        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-gray-900 tracking-tight">Update Dispatch Details</h2>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4">
              {/* Vehicle Selection */}
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-2 ml-1">Response Asset / Vehicle</label>
                <div className="relative">
                    <Car className="absolute left-3 top-3.5 text-gray-400" size={20}/>
                    <select
                        value={selectedVehicle}
                        onChange={(e) => setSelectedVehicle(e.target.value)}
                        className="w-full bg-gray-100/50 border border-gray-200 rounded-2xl pl-10 pr-4 py-3 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all text-gray-800"
                    >
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-2 ml-1">Officers in Charge (Dispatched)</label>
                ...
                <div className="flex space-x-2 mb-3">
                    <div className="relative flex-1">
                        ...
                        >
                            ...
                            {availableOfficers.map(officer => (
                                <option key={officer.id} value={officer.full_name}>
                    ...
                    <button 
                        ...
                        className="bg-slate-800 text-white rounded-2xl px-4 flex items-center justify-center disabled:opacity-50 hover:bg-slate-900"
                    >
                        <Plus size={20} />
                    </button>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                    {selectedOfficers.length === 0 && (
                        <span className="text-xs text-gray-400 italic pl-1">No officers assigned yet.</span>
                    )}
                    {selectedOfficers.map(officer => (
                        <span key={officer} className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700 border border-blue-200">
                            {officer}
                            <button onClick={() => handleRemoveOfficer(officer)} className="ml-2 hover:text-red-500">
                                <X size={12} />
                            </button>
                        </span>
                    ))}
          ...
          <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100 flex items-start space-x-3">
               ...
               <div>
                   <p className="text-xs font-bold text-blue-700 uppercase">Unit Designation Preview</p>
          ...
          {/* Status Selection */}
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-2 ml-1">Current Status</label>
          ...
          <button
            onClick={handleSave}
            disabled={loading}
            className="w-full bg-black text-white rounded-2xl py-4 font-semibold text-lg hover:scale-[1.02] active:scale-[0.98] transition-transform duration-200 flex items-center justify-center space-x-2 shadow-xl"
          >
            {loading ? (
