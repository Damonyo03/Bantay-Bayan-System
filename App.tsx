        ...
        <div className="min-h-screen flex items-center justify-center p-4">
             <div className="w-full max-w-md bg-white rounded-[2rem] p-8 shadow-2xl">
                 ...
                 <form onSubmit={handleUpdate} className="space-y-4">
                     <div>
                         <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Enter New Password</label>
                         <input 
                            type="password"
                            required
                            minLength={8}
                            className="w-full bg-gray-100 rounded-xl py-3 px-4 outline-none border border-gray-200"
                            value={password}
                     ...
                     <button 
                        type="submit" 
                        disabled={loading}
                        className="w-full bg-slate-900 text-white font-bold py-3 rounded-xl flex items-center justify-center space-x-2"
                     >
...
const AppContent: React.FC = () => {
    ...
    if (!user) {
        return (
             <Routes>
                ...
                {/* Allow accessing update-password even if 'not logged in' via context, because Supabase handles session recovery */}
    ...
    return (
        <div className="min-h-screen text-slate-800 font-sans antialiased selection:bg-blue-100 selection:text-blue-900">
            <Sidebar />
            {/* 
                Responsive Layout:
                - Mobile: pb-24 (Space for Bottom Nav), pl-0
                - Desktop (md): pb-8, pl-64 (Space for Sidebar)
            */}
