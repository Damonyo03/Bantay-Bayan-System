    ...
    <div className="max-w-5xl mx-auto pb-20">
      <header className="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        <div>
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight">Official Blotter</h1>
            <p className="text-slate-300 mt-2">Log incident details for official barangay records.</p>
        </div>
        <div className="hidden md:block text-right">
            <p className="text-sm font-bold text-slate-400 uppercase">Officer on Duty</p>
            <p className="text-lg font-semibold text-white">{user?.full_name}</p>
            <p className="text-xs text-slate-400 font-mono">{user?.badge_number}</p>
        </div>
      </header>
      ...
      <div className="flex items-center space-x-4 mb-8">
        {[1, 2, 3].map((s) => (
          <div key={s} className={`h-1.5 rounded-full flex-1 transition-all duration-500 ${step >= s ? 'bg-white' : 'bg-white/20'}`} />
        ))}
      </div>
      ...
      <div className="glass-panel rounded-2xl sm:rounded-[2rem] p-4 sm:p-8 shadow-xl min-h-[500px] relative overflow-hidden">
        ...
        {step === 1 && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Category (Uri)</label>
                <select 
                  className="w-full bg-white/60 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-slate-500/20 outline-none transition-all font-medium text-slate-800"
                  value={formData.type}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Location (Lugar)</label>
                <input 
                  type="text" 
                  className="w-full bg-white/60 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-slate-500/20 outline-none text-slate-800"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Action Taken</label>
                <select 
                  className="w-full bg-white/60 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-slate-500/20 outline-none text-slate-800"
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Narrative (Salaysay)</label>
              <textarea 
                className="w-full bg-white/60 border border-gray-200 rounded-xl px-4 py-3 h-40 focus:ring-2 focus:ring-slate-500/20 outline-none resize-none font-normal leading-relaxed text-slate-800"
        ...
        {step === 2 && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {parties.length === 0 && (
                <div className="text-center py-12 text-slate-400 border-2 border-dashed border-gray-300 rounded-2xl bg-gray-50/50">
                  No parties recorded.
                </div>
              )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                    <div className="md:col-span-4">
                      ...
                      <input 
                        ...
                        className="w-full bg-white/80 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none font-semibold text-slate-800"
                        placeholder="Last, First MI"
                      />
                    </div>
                    <div className="md:col-span-2">
                      <label className="text-xs text-slate-500 font-bold uppercase mb-1 block">Age</label>
                      <input 
                        type="number" 
                        value={party.age || ''}
                        onChange={(e) => handlePartyChange(idx, 'age', parseInt(e.target.value))}
                        className="w-full bg-white/80 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none text-slate-800"
                      />
                    </div>
                    ...
                    <div className="md:col-span-3">
                        <label className="text-xs text-slate-500 font-bold uppercase mb-1 block">Contact #</label>
                        <input 
                            type="text" 
                    ...
                    <div className="md:col-span-12">
                      <label className="text-xs text-slate-500 font-bold uppercase mb-1 block">Statement / Remarks</label>
                      <input 
                        type="text" 
            ...
            <div className="flex justify-between pt-6 border-t border-gray-200/50">
               <button 
                onClick={() => setStep(1)}
                className="text-slate-500 hover:text-slate-900 font-medium transition-colors px-4"
              >
                Back
              </button>
              <button 
        ...
        {step === 3 && (
            <div className="flex flex-col items-center justify-center py-12 animate-fade-in text-center">
                ...
                <h2 className="text-3xl font-bold text-slate-900 mb-2">Blotter Filed Successfully</h2>
                <p className="text-slate-500 mb-2">Case Number: <span className="font-mono font-bold text-slate-800">{lastIncident?.case_number}</span></p>
                <p className="text-slate-500 mb-8 max-w-md">The incident has been officially recorded in the barangay database.</p>
