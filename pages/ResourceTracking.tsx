...
import { generateBorrowingSlip, reprintCCTVForm } from '../utils/pdfGenerator';
...
const ResourceTracking: React.FC = () => {
  ...
  const fetchCCTV = async () => {
      try {
          const data = await supabaseService.getCCTVRequests();
  ...
  const handleLogVehicleUsage = async (e: React.FormEvent) => {
      ...
      try {
          ...
          await supabaseService.logVehicleDispatch(user.id, vehicleForm.vehicle, officersStr, vehicleForm.purpose, vehicleForm.destination);
  ...
  return (
    <div className="space-y-8 pb-20 animate-fade-in">
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        ...
        <button onClick={() => navigate('/resources/new')} className="w-full sm:w-auto mt-2 sm:mt-0 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 py-3 rounded-xl font-bold hover:scale-105 transition-transform shadow-xl flex items-center justify-center space-x-2">
            <Plus size={18} />
      ...
      <div className="flex space-x-2 bg-gray-200/50 dark:bg-white/5 p-1.5 rounded-2xl w-full sm:w-fit overflow-x-auto no-scrollbar">
          <button onClick={() => setFilter('Pending')} className={`flex-shrink-0 px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center space-x-2 whitespace-nowrap ${filter === 'Pending' ? 'bg-white dark:bg-slate-800 text-purple-700 dark:text-purple-400 shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'}`}>
              <Clock size={16} />
              <span>Pending</span>
              {counts.Pending > 0 && <span className="ml-2 px-2 py-0.5 bg-red-500 text-white rounded-full text-[10px]">{counts.Pending}</span>}
          </button>
          <button onClick={() => setFilter('Scheduled')} className={`flex-shrink-0 px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center space-x-2 whitespace-nowrap ${filter === 'Scheduled' ? 'bg-white dark:bg-slate-800 text-blue-700 dark:text-blue-400 shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'}`}>
              <CalendarCheck size={16} />
              <span>Approved</span>
              {counts.Scheduled > 0 && <span className="ml-2 px-2 py-0.5 bg-blue-500 text-white rounded-full text-[10px]">{counts.Scheduled}</span>}
          </button>
          <button onClick={() => setFilter('History')} className={`flex-shrink-0 px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center space-x-2 whitespace-nowrap ${filter === 'History' ? 'bg-white dark:bg-slate-800 text-slate-800 dark:text-white shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'}`}>
              <Archive size={16} />
              <span>History</span>
          </button>
          <button onClick={() => setFilter('CCTV')} className={`flex-shrink-0 px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center space-x-2 whitespace-nowrap ${filter === 'CCTV' ? 'bg-white dark:bg-slate-800 text-red-600 dark:text-red-400 shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'}`}>
              <Video size={16} />
              <span>CCTV Logs</span>
              {counts.CCTV > 0 && <span className="ml-2 px-2 py-0.5 bg-red-600 text-white rounded-full text-[10px]">{counts.CCTV}</span>}
          </button>
          <button onClick={() => setFilter('Vehicles')} className={`flex-shrink-0 px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center space-x-2 whitespace-nowrap ${filter === 'Vehicles' ? 'bg-white dark:bg-slate-800 text-orange-600 dark:text-orange-400 shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'}`}>
              <Car size={16} />
                 ...
                 <div className="glass-panel rounded-3xl overflow-hidden shadow-xl border border-white/50 dark:border-white/10">
                    <div className="p-4 bg-gray-50/50 dark:bg-white/5 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
                        ...
                        <div className="flex items-center space-x-2">
                             <button onClick={() => setIsVehicleModalOpen(true)} className="flex items-center space-x-2 px-4 py-2 bg-orange-600 text-white rounded-xl text-xs font-bold hover:bg-orange-700 transition-colors shadow-lg">
                                 <Navigation size={14} />
                                 <span>Log Usage</span>
                             </button>
                        </div>
                            ...
                            <div className="flex flex-col lg:flex-row justify-between lg:items-start gap-6 pl-4">
                                ...
                                <div className="flex flex-col space-y-3 lg:w-56 pt-2">
                                    ...
                                    {user?.role === 'supervisor' && req.status === 'Approved' && (
                                        <button onClick={() => handleStatusUpdate(req.id, 'Released')} className="w-full py-2.5 bg-purple-600 text-white rounded-xl text-xs font-bold">Mark as Released</button>
                                    )}
                                    {user?.role === 'supervisor' && req.status === 'Released' && (
                                        <button onClick={() => handleStatusUpdate(req.id, 'Returned')} className="w-full py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold">Mark as Returned</button>
                                    )}
                                </div>
                            </div>
